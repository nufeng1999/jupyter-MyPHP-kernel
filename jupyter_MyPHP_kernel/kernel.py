...
class MyPHPKernel(MyKernel):
    implementation = 'jupyter-MyPHP-kernel'
    implementation_version = '1.0'
    language = 'PHP'
    language_version = ''
    language_info = {'name': 'PHP',
...
##//%include:src/comm_attribute.py
    def __init__(self, *args, **kwargs):
        super(MyPHPKernel, self).__init__(*args, **kwargs)
        self.runfiletype='script'
        self.kernelinfo="[MyPHPKernel{0}]".format(time.strftime("%H%M%S", time.localtime()))
        
#################
    def compile_with_phpc(self, source_filename, binary_filename, cflags=None, ldflags=None,env=None,magics=None):
    ...
    def _exec_phpc_(self,source_filename,magics):
        self.mymagics._logln('Generating executable file')
        with self.mymagics.new_temp_file(suffix='.php') as binary_file:
            p,outfile,tsccmd = self.compile_with_phpc(
                source_filename, 
                binary_file.name,
...
##do_runcode
    def do_runcode(self,return_code,fil_ename,magics,code, silent, store_history=True,
        ...
        retstr=''
        ##代码运行前
        p = self.mymagics.create_jupyter_subprocess(['php','-f',fil_ename]+ magics['_st']['args'],cwd=None,shell=False,env=self.mymagics.addkey2dict(magics,'env'),magics=magics)
...
##do_compile_code
    def do_compile_code(self,return_code,fil_ename,magics,code, silent, store_history=True,
        ...
        retinfo=self.mymagics.get_retinfo()
        retstr=''
        # returncode,binary_filename=self._exec_phpc_(fil_ename,magics)
...
##do_create_codefile
    def do_create_codefile(self,magics,code, silent, store_history=True,
        ...
        retinfo=self.mymagics.get_retinfo()
        retstr=''
        source_file=self.mymagics.create_codetemp_file(magics,code,suffix='.php')
